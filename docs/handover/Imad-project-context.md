...
**Last updated**: 2026-09-22, at session handoff (context transfer). MiMo-V2.6-Pro is Architect (decision-making); MiMo-V2.6-Flash is Implementer (code/deploy); Claude[Sonnet] Web is Reviewer (every handover); ChatGPT is Co-reviewer (critical changes). See `AGENTS.md` and `opencode.json` at project root for the onboarding brief and auto-load configuration.
